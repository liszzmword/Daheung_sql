...
- product_spec: text (규격/사이즈, 예: 1,200X50, 100X50)
- product_group: text (제품군)
- qty: numeric (수량)
- unit_price: numeric (판매 단가)
- supply_amount: numeric (공급가액, 부가세 제외) ← 매출 계산 시 사용
- margin_rate_pct: numeric (마진율, 소수점 형식: 0.17 = 17%)
- vat: numeric (부가세, 레거시 데이터만 존재)
- total_amount: numeric (합계, 레거시 데이터만 존재)
