 ...
 * - normalized_name은 normalizeName(company_name)으로 자동 채움
 * - aliases는 기존 값을 보존 (upsert 시 덮어쓰지 않으려면 별도 관리)
 */
...
/**
 * 한글 법인 접두어/접미어 패턴
 * - 매칭 시 검색어와 저장된 이름 양쪽에서 제거
 */
const COMPANY_PREFIXES = /주식회사|유한회사|합자회사|합명회사|협동조합|사단법인|재단법인|㈜|\(주\)|（주）|\(유\)|（유）/g;

/**
 * 이름 정규화 (법인 접두어/공백/괄호 제거, 소문자)
 * 예: "주식회사 쓰리에이치" → "쓰리에이치"
 *     "(주)대영케미칼상사" → "대영케미칼상사"
 */
