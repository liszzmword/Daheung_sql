...
import { TABLE_SCHEMA, DIARY_TABLE_SCHEMA, CUSTOMERS_TABLE_SCHEMA } from "./schema.mjs";
...
# 테이블 선택 규칙 (매우 중요)
- 매출, 금액, 거래처별 실적, 제품별 판매 관련 질문 → sales_clean 테이블 사용
- 영업일지, 방문기록, 상담내용, 미팅내용 관련 질문 → sales_diary 테이블 사용
- 거래처 정보 (사업자번호, 담당자, 부서 등) 질문 → customers 테이블 사용
- 두 테이블을 함께 사용해야 할 경우 (예: 방문 횟수와 매출 상관관계) → JOIN 가능
  - JOIN 키: customer_code로 연결 (가장 정확)
  - 또는: sales_diary.company_name과 sales_clean.customer_name이 대응
  - sales_diary.sales_rep과 sales_clean.sales_rep이 대응
- customers 테이블과 JOIN: sales_clean.customer_code = customers.customer_code
...
- vat, total_amount 컬럼은 사용하지 않는다 (데이터 없음)
- margin_rate_pct는 소수점 형식 (0.17 = 17%). 퍼센트로 표시할 때 * 100 필요
