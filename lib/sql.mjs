...
function cleanSQL(raw) {
  ...
  // 4. SQL 끝 이후 한글 설명문 제거
  // 주의: CTE 이름은 영문이므로 [A-Za-z] 매칭은 안전하지 않음 — 한글로 시작하는 설명만 제거
  sql = sql.replace(/\n\s*\n\s*[가-힣][\s\S]*$/, "");
...
# 거래처명 검색 규칙 (매우 중요 - 반드시 따를 것)
- 거래처명은 customers 테이블의 normalized_name과 aliases를 통해 customer_code로 해소한 뒤,
  sales_clean / purchases / sales_diary에서 customer_code IN (...)로 필터링
- 절대로 sales_clean.customer_name이나 purchases.customer_name에 직접 LIKE/=를 적용 금지
  (단, purchases는 공급업체이므로 매입 분석 시에는 purchases.customer_name 직접 검색 가능)
- 검색어 정규화: regexp_replace(lower('검색어'), '주식회사|유한회사|합자회사|합명회사|협동조합|사단법인|재단법인|㈜|\\(주\\)|（주）|\\s', '', 'g')
- 거래처명 검색 예시 (sales_clean에서 매출 조회):
  WITH search_norm AS (
    SELECT regexp_replace(
      lower('검색어'),
      '주식회사|유한회사|합자회사|합명회사|협동조합|사단법인|재단법인|㈜|\\(주\\)|（주）|\\s',
      '', 'g'
    ) AS q
  ),
  matched_customers AS (
    SELECT customer_code, company_name FROM (
      SELECT c.customer_code, c.company_name,
        CASE
          WHEN c.normalized_name = (SELECT q FROM search_norm) THEN 0
          WHEN (SELECT q FROM search_norm) = ANY(c.aliases) THEN 0
          WHEN c.normalized_name LIKE '%' || (SELECT q FROM search_norm) || '%' THEN 1
          ELSE 2
        END AS priority,
        similarity(c.normalized_name, (SELECT q FROM search_norm)) AS sim
      FROM customers c
      WHERE c.normalized_name LIKE '%' || (SELECT q FROM search_norm) || '%'
         OR (SELECT q FROM search_norm) = ANY(c.aliases)
         OR similarity(c.normalized_name, (SELECT q FROM search_norm)) > 0.3
    ) t
    ORDER BY priority, sim DESC
    LIMIT 1
  )
  SELECT ... FROM sales_clean
  WHERE customer_code IN (SELECT customer_code FROM matched_customers)
- sales_diary, purchases도 동일한 customers 해소 패턴 사용 후 customer_code IN (...) 적용
- 제품명(product_name) 검색은 기존 패턴 유지 (sales_clean에서 직접 similarity()):
  WITH matched_product AS (
    SELECT product_name FROM (
      SELECT DISTINCT product_name,
        CASE WHEN product_name LIKE '%검색어%' THEN 0 ELSE 1 END AS priority,
        similarity(product_name, '검색어') AS sim
      FROM sales_clean
      WHERE product_name LIKE '%검색어%'
         OR similarity(product_name, '검색어') > 0.3
    ) t
    ORDER BY priority, sim DESC
    LIMIT 1
  )
