/**
 * 파일 업로드 API (세일즈 CSV/XLSX / 매입 CSV / 영업일지 XLS / 거래처 마스터 XLS)
 * POST /api/upload
 * multipart/form-data: file, type(sales|purchases|diary|customers)
 */
...
/** 날짜 변환 (다양한 형식 지원, fallbackYear: MM/DD만 있을 때 사용할 연도) */
function parseDate(val, fallbackYear = null) {
  ...
  // MM/DD 형식 (06/24) - 연도 없음, fallbackYear 사용
...
/** 세일즈 CSV 처리 (한글 15컬럼: 번호,매출일,거래처,담당사원,제품명,규격(재단),제품군,수량,매입단가(3%),판매단가,공급가액,부가세,합계,마진율,스탁번호) */
async function processSalesCSV(buffer, filename) {
  ...
  if (records.length === 0) throw new Error("유효한 데이터가 없습니다.");
...
/** 매입 CSV 처리 (한글 12컬럼, 날짜 MM/DD → 파일명에서 연도 추출) */
async function processPurchaseCSV(buffer, filename) {
  // 파일명에서 연도 추출 (매입내역_2020.csv → 2020)
  ...
  // 파일 단위 교체
  await supabase.from("purchases").delete().eq("source_file", filename);
  ...
  await logSync("purchases_csv", filename, records.length, inserted);
...
async function processSalesXLSX(buffer, filename) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    for (let i = 0; i < headers.length; i++) {
      const h = headers[i];
      const mapped = SALES_COLUMN_MAP[h];
      if (mapped) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 타입 검증
    if (!type || !["sales", "purchases", "diary", "customers"].includes(type)) {
      return res.status(400).json({
        success: false,
        error: 'type은 "sales", "purchases", "diary", 또는 "customers"이어야 합니다.',
      });
    }
    ...
    if (type === "customers") {
      ...
      if (ext !== ".csv") {
        return res.status(400).json({ success: false, error: "매입 데이터는 CSV 파일만 지원합니다." });
      }
      result = await processPurchaseCSV(buffer, filename);
    } else if (type === "sales") {
      if (ext === ".csv") {
